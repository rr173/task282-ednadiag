...
func TestIsolatedBatchStopsPropagation(t *testing.T) {
	app := newProbeApp(t)
	chainID := seedContamChain(t, app)
	if _, err := app.Prop.Analyze(chainID); err != nil {
		t.Fatal(err)
	}
	if err := app.Store.SetBatchIsolated("B1", true); err != nil {
		t.Fatal(err)
	}
	paths, err := app.Prop.Analyze(chainID)
	if err != nil {
		t.Fatal(err)
	}
	for _, p := range paths {
		if p.ViaBatchID == "B1" {
			t.Fatalf("isolated batch still propagates: %+v", p)
		}
	}
	_ = propagation.NewService
	_ = model.PathCandidate
}
