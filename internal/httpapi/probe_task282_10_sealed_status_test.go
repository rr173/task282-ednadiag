...
func TestSealImmutableMapsConflict(t *testing.T) {
	app := newProbeApp(t)
	chainID := seedContamChain(t, app)
	if err := app.RunTrace(chainID); err != nil {
		t.Fatal(err)
	}
	if err := app.Chain.Publish(chainID); err != nil {
		t.Fatal(err)
	}
	if err := app.Chain.Seal(chainID); err != nil {
		t.Fatal(err)
	}
	h := httpapi.NewServer(app)
	req := httptest.NewRequest(http.MethodPost, "/api/chains/"+chainID+"/trace", nil)
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	if rec.Code != http.StatusConflict {
		t.Fatalf("sealed trace status=%d body=%s", rec.Code, rec.Body.String())
	}
	_ = model.ErrSealedImmutable
}
